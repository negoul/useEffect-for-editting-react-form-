...
import {BASE_URL, FILE_TOKEN} from '../config/variables.config';
...
* @param {String} fileToken - The fileToken
* */
export function getFile(id){
  if (!id) return;
  const fileToken = JSON.parse(localStorage.getItem(FILE_TOKEN));
  return `${BASE_URL}/api/backOffice/customers/define/files/get/${id}?fileToken=${fileToken}`;
}
...
export function getAvatar(id){
  if (!id) return;
  const fileToken = JSON.parse(localStorage.getItem(FILE_TOKEN));
  return `${BASE_URL}/api/user/profile/avatar/${id}?fileToken=${fileToken}`;
}
...
* */
export function jalaliDate(timestamp, format = 'HH:mm:ss jYYYY/jMM/jDD ', milisecond = true) {
...
export function getAppDirection() {
  const dir = document.head.parentElement.getAttribute('dir');
  return dir ? dir : 'ltr';
}
...
export function createFilterQueryString(filter) {
  let qs = '';
...
export function objectPropsCount(data) {
  return !isEmptyObject(data) ? Object.values(data).filter(item => !!item).length : 0;
}
...
export function isEmptyObjectProperties(data) {
  return isEmptyObject(data) ? true : Object.values(data).every(x => (x === null || x === ''))
}
...
export const toast = {
  ...
  createToast(option, resolve, reject) {
    ...
    const toastEl = createElement('section', '', {id: `${this.preClass}-${this.count}`,className: `${this.preClass} ${this.preClass}--${type} ${isRTL ? 'rtl' : 'ltr'} ${isAbsolute ? this.preClass + '--absolute' : ''}`});
